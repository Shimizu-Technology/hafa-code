...
The Playwright gate runs axe against personal and classroom workspaces in default and dark color-safe modes. It also checks every mobile destination plus the Guide and Practice overlays in explicit Light and Dark modes, including selected and transient state contrast. Keyboard activation and focus behavior cover sharing, projects, files, history, and classroom tabs; layout checks cover a 640 CSS-pixel viewport as the equivalent of a 1280-pixel display at 200% browser zoom, horizontal overflow at mobile sizes, and 44-pixel primary mobile targets. These checks reduce regressions, but they do not replace hands-on VoiceOver/other screen-reader testing or validation on the school's actual devices and network.
