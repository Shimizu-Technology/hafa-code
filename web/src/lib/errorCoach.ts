...
const sharedRules: Partial<Record<ProjectKind, readonly ErrorRule[]>> = {
  ...
  sql: [
    ...
    { pattern: /no such column/i, title: 'SQLite cannot find that column', explanation: 'The column is missing, misspelled, or needs a table alias.', steps: ['Compare the column with schema.sql.', 'Check the table alias before the dot.', 'Run a simple SELECT * to inspect the available columns.'], topic: 'sql-select' },
