...
export function MobileWorkspaceNav({ activeTab, projectKind, onChange }: MobileWorkspaceNavProps) {
  return (
    <nav className="mobile-bottom-nav" aria-label="Workspace sections">
      <button className={activeTab === 'home' ? 'active' : ''} type="button" onClick={() => onChange('home')}>
        <Layers3 size={18} />
        <span>Home</span>
      </button>
      <button className={activeTab === 'projects' ? 'active' : ''} type="button" onClick={() => onChange('projects')}>
        <Files size={18} />
        <span>Projects</span>
      </button>
      <button className={activeTab === 'code' ? 'active' : ''} type="button" onClick={() => onChange('code')}>
        <BookOpen size={18} />
        <span>Code</span>
      </button>
      <button className={activeTab === 'output' ? 'active' : ''} type="button" onClick={() => onChange('output')}>
        {projectKind === 'web' ? <Globe size={18} /> : <Terminal size={18} />}
        <span>{projectKind === 'web' ? 'Preview' : 'Output'}</span>
      </button>
      <button className={activeTab === 'history' ? 'active' : ''} type="button" onClick={() => onChange('history')}>
        <History size={18} />
        <span>History</span>
      </button>
    </nav>
